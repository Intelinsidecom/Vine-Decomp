...
class SessionDataWriter {
    ...
    private int getSessionAppSize(ByteString packageName, ByteString versionCode, ByteString versionName, ByteString installUuid, int deliveryMechanism) {
        ...
        return size2 + CodedOutputStream.computeTagSize(5) + CodedOutputStream.computeRawVarint32Size(orgSize) + orgSize + CodedOutputStream.computeBytesSize(6, installUuid) + CodedOutputStream.computeEnumSize(10, deliveryMechanism);
    }

    private int getSessionAppOrgSize() {
        int size = 0 + CodedOutputStream.computeBytesSize(1, ByteString.copyFromUtf8(new ApiKey().getValue(this.context)));
        return size;
    }
    ...
    private int getBinaryImageSize() {
        int size = 0 + CodedOutputStream.computeUInt64Size(1, 0L) + CodedOutputStream.computeUInt64Size(2, 0L) + CodedOutputStream.computeBytesSize(3, this.packageNameBytes);
        if (this.optionalBuildIdBytes != null) {
    ...
    private int getEventAppExecutionSize(Thread exceptionThread, Throwable ex) {
        ...
        for (int i = 0; i < len; i++) {
            Thread thread = this.threads[i];
            int threadSize2 = getThreadSize(thread, this.stacks.get(i), 0, false);
    ...
    private int getEventDeviceSize(float batteryLevel, int batterVelocity, boolean proximityEnabled, int orientation, long heapAllocatedSize, long diskUsed) {
        int size = 0 + CodedOutputStream.computeFloatSize(1, batteryLevel);
        return size + CodedOutputStream.computeSInt32Size(2, batterVelocity) + CodedOutputStream.computeBoolSize(3, proximityEnabled) + CodedOutputStream.computeUInt32Size(4, orientation) + CodedOutputStream.computeUInt64Size(5, heapAllocatedSize) + CodedOutputStream.computeUInt64Size(6, diskUsed);
    }
