...
final class Id3Util {
    ...
    public static GaplessInfo parseId3(ExtractorInput input) throws InterruptedException, IOException {
    ...
    private static GaplessInfo parseGaplessInfo(ParsableByteArray frame, int version, int flags) {
        ...
        if (version == 3 && (flags & 64) != 0) {
            if (frame.bytesLeft() >= 4 && (extendedHeaderSize = frame.readUnsignedIntToInt()) <= frame.bytesLeft()) {
        ...
        if (version == 4 && (flags & 64) != 0) {
            ...
            int extendedHeaderSize2 = frame.readSynchSafeInt();
            if (extendedHeaderSize2 < 6 || extendedHeaderSize2 > frame.bytesLeft() + 4) {
                return null;
            }
            frame.setPosition(extendedHeaderSize2);
        }
        while (true) {
            ...
            if (majorVersion == 2) {
                ...
                if (!id2.equals("\u0000\u0000\u0000\u0000")) {
                    ...
                    if (frameSize != 0 && frameSize <= data.bytesLeft() - 2) {
                        ...
                        boolean compressedOrEncrypted = (majorVersion == 4 && (flags & 12) != 0) || (majorVersion == 3 && (flags & 192) != 0);
        ...
        if (version != 4) {
            if ((flags & 128) != 0) {
                ...
                for (int i = 0; i + 1 < newLength; i++) {
                    if ((bytes[i] & Constants.UNKNOWN) == 255 && bytes[i + 1] == 0) {
    ...
    private static void unescapeVersion4(ParsableByteArray frame, boolean unsignedIntDataSizeHack) {
        ...
        while (frame.bytesLeft() >= 10 && frame.readInt() != 0) {
            ...
            if ((flags & 2) != 0) {
                ...
                while (i + 1 < dataSize) {
                    if ((bytes[readOffset - 1] & Constants.UNKNOWN) == 255 && bytes[readOffset] == 0) {
                        readOffset++;
                        dataSize--;
                    }
