...
final class BulkFollowTwitterActionNotifier implements ListenerNotifier {
    private final ArrayList<UserInteractionsListener> mListeners;

    public BulkFollowTwitterActionNotifier(ArrayList<UserInteractionsListener> listeners) {
    ...
    public void notifyListeners(ServiceNotification notification) {
        ...
        while (it.hasNext()) {
            ...
            listener.onBulkFollowTwitterComplete(notification.reqId, notification.statusCode, notification.reasonPhrase);
        }
    }
}
