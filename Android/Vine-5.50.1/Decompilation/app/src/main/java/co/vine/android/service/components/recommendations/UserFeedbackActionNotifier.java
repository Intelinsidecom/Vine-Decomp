...
public final class UserFeedbackActionNotifier implements ListenerNotifier {
    private final ArrayList<RecommendationsListener> mListeners;

    public UserFeedbackActionNotifier(ArrayList<RecommendationsListener> listeners) {
    ...
    public void notifyListeners(ServiceNotification notification) {
        ...
        while (it.hasNext()) {
            ...
            listener.onUserFeedbackComplete(notification.reqId, notification.statusCode, notification.reasonPhrase, userId, recBoost);
        }
    }
}
