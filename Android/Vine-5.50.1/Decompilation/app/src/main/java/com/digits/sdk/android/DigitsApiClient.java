...
class DigitsApiClient {
    ...
    DigitsApiClient(Session session) {
        this(session, TwitterCore.getInstance().getAuthConfig(), TwitterCore.getInstance().getSSLSocketFactory(), Digits.getInstance().getExecutorService());
    }

    DigitsApiClient(Session session, TwitterAuthConfig authConfig, SSLSocketFactory sslFactory, ExecutorService executorService) {
        this(session, authConfig, sslFactory, executorService, new DigitsUserAgent());
    }
