package com.tune.crosspromo;

import android.content.Context;
import android.graphics.Bitmap;
import android.graphics.BitmapFactory;
import android.graphics.Rect;
import android.graphics.drawable.BitmapDrawable;
import android.util.Base64;
import android.util.DisplayMetrics;
import android.view.TouchDelegate;
import android.view.View;
import android.widget.FrameLayout;
import android.widget.ImageButton;

/* loaded from: classes.dex */
class TuneCloseButton extends FrameLayout {
    private TuneAdActivity adActivity;
    private ImageButton close;
    View.OnClickListener closeListener;

    public TuneCloseButton(Context context) {
        super(context);
        this.closeListener = new View.OnClickListener() { // from class: com.tune.crosspromo.TuneCloseButton.1
            @Override // android.view.View.OnClickListener
            public void onClick(View v) {
                TuneAdClient.logClose(TuneCloseButton.this.adActivity.adView, TuneCloseButton.this.adActivity.adParams);
                TuneCloseButton.this.adActivity.finish();
            }
        };
        this.adActivity = (TuneAdActivity) context;
        this.close = new ImageButton(context);
        DisplayMetrics dm = getResources().getDisplayMetrics();
        byte[] decodedString = Base64.decode("", 0);
        Bitmap decodedByte = BitmapFactory.decodeByteArray(decodedString, 0, decodedString.length);
        BitmapDrawable bd = new BitmapDrawable(getResources(), decodedByte);
        this.close.setBackgroundDrawable(bd);
        final float density = dm.density;
        int btnSize = (int) ((36.0f * density) + 0.5d);
        int marginSize = (int) ((8.0f * density) + 0.5d);
        FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(btnSize, btnSize);
        params.gravity = 53;
        params.setMargins(0, marginSize, marginSize, 0);
        this.close.setLayoutParams(params);
        this.close.setOnClickListener(this.closeListener);
        addView(this.close);
        post(new Runnable() { // from class: com.tune.crosspromo.TuneCloseButton.2
            @Override // java.lang.Runnable
            public void run() {
                Rect delegateArea = new Rect();
                TuneCloseButton.this.close.getHitRect(delegateArea);
                int touchPadding = (int) ((12.0f * density) + 0.5d);
                delegateArea.top -= touchPadding;
                delegateArea.left -= touchPadding;
                delegateArea.bottom += touchPadding;
                delegateArea.right += touchPadding;
                TouchDelegate expandedArea = new TouchDelegate(delegateArea, TuneCloseButton.this.close);
                if (View.class.isInstance(TuneCloseButton.this.close.getParent())) {
                    ((View) TuneCloseButton.this.close.getParent()).setTouchDelegate(expandedArea);
                }
            }
        });
    }
}
