...
public class DefaultMp4Builder {
    ...
    public Container build(Movie movie) {
        ...
        for (SampleAuxiliaryInformationOffsetsBox saio : this.sampleAuxiliaryInformationOffsetsBoxes) {
            ...
            Object current = saio;
            while (true) {
                Container b = ((Box) current).getParent();
                Iterator<Box> it = b.getBoxes().iterator();
    ...
    protected void createStco(Track track, Movie movie, Map<Track, int[]> chunks, SampleTableBox stbl) {
        ...
        for (int i = 0; i < tracksChunkSizes.length; i++) {
            ...
            for (Track current : movie.getTracks()) {
                ...
                for (int j2 = CastUtils.l2i(firstSampleOfChunk); j2 < chunkSizes[i] + firstSampleOfChunk; j2++) {
    ...
    private class InterleaveChunkMdat implements Box {
        ...
        private InterleaveChunkMdat(Movie movie, Map<Track, int[]> chunks, long contentSize) {
            ...
            for (int i = 0; i < chunks.values().iterator().next().length; i++) {
                for (Track track : this.tracks) {
                    ...
                    List<Sample> chunk = DefaultMp4Builder.this.track2Sample.get(track).subList(CastUtils.l2i(firstSampleOfChunk), CastUtils.l2i(chunkSizes[i] + firstSampleOfChunk));
        ...
        public long getDataOffset() {
            Box box;
            long offset = 16;
            for (Object parent = this; parent instanceof Box; parent = ((Box) parent).getParent()) {
                Iterator<Box> it = ((Box) parent).getParent().getBoxes().iterator();
                while (it.hasNext() && parent != (box = it.next())) {
                    offset += box.getSize();
                }
            }
            return offset;
        ...
        private boolean isSmallBox(long contentSize) {
            return 8 + contentSize < avutil.AV_CH_WIDE_RIGHT;
        }
