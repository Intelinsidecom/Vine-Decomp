...
public class DraftViewPager extends ViewGroup {
    ...
    public static final int DRAFT_MARGIN = 6;
    private static final int INVALID_POINTER = -1;
    private static final int MAX_SETTLE_DURATION = 600;
    private static final int MIN_DISTANCE_FOR_FLING = 25;
    public static final int PREVIEW_RATIO = 85;
    ...
    private static final Comparator<ItemInfo> COMPARATOR = new Comparator<ItemInfo>() { // from class: android.support.v4.view.DraftViewPager.1
        @Override // java.util.Comparator
        public int compare(ItemInfo lhs, ItemInfo rhs) {
            return lhs.position - rhs.position;
        }
    };
    private static final Interpolator sInterpolator = new Interpolator() { // from class: android.support.v4.view.DraftViewPager.2
    ...
    interface OnAdapterChangeListener {
        void onAdapterChanged(DraftPagerAdapter draftPagerAdapter, DraftPagerAdapter draftPagerAdapter2);
    }
    ...
    public DraftViewPager(Context c) {
        this(c, null);
    }

    public DraftViewPager(Context c, AttributeSet attrs) {
        super(c, attrs);
    ...
    public void setAdapter(DraftPagerAdapter adapter) {
        ...
        DraftPagerAdapter oldAdapter = this.mAdapter;
    ...
    void setCurrentItemInternal(int item, boolean smoothScroll, boolean always, int velocity) {
        ...
        this.mCurItem = item;
        populate();
        int destX = getAdjustedXDestination(item);
        if (smoothScroll) {
            smoothScrollTo(item, destX, 0, velocity);
    ...
    private int getAdjustedXDestination(int item) {
        int base = ((int) (getWidth() * this.mPreviewRatio)) + (this.mDraftMargin * 2);
        return base * item;
    }
    ...
    void smoothScrollTo(int position, int x, int y, int velocity) {
        int duration;
        if (getChildCount() == 0) {
            setScrollingCacheEnabled(false);
            return;
    ...
    public static class SavedState extends View.BaseSavedState {
        public static final Parcelable.Creator<SavedState> CREATOR = ParcelableCompat.newCreator(new ParcelableCompatCreatorCallbacks<SavedState>() { // from class: android.support.v4.view.DraftViewPager.SavedState.1
            ...
            if (child.getVisibility() != 8) {
                ...
                if (lp.isDecor) {
                    ...
                    if (ii != null) {
                        ...
                        int rawOffset = (int) (getWidth() * (1.0f - this.mPreviewRatio));
                        int firstChildOffset = (int) (getWidth() * ((1.0f - this.mPreviewRatio) / 2.0f));
                        if (ii.position > 0) {
                            int offset = rawOffset * ii.position;
                            int margin = this.mDraftMargin * 2 * ii.position;
                            child.layout((childLeft3 - offset) + firstChildOffset + margin, childTop2, ((child.getMeasuredWidth() + childLeft3) - offset) + firstChildOffset + margin, child.getMeasuredHeight() + childTop2);
                        } else {
                            child.layout(childLeft3 + firstChildOffset, childTop2, child.getMeasuredWidth() + childLeft3 + firstChildOffset, child.getMeasuredHeight() + childTop2);
                        }
                    }
                }
            }
        ...
        switch (action & 255) {
            ...
            case 1:
                if (this.mIsBeingDragged) {
                    ...
                    int widthWithMargin = getWidth() + this.mPageMargin;
                    int currentPage = getScrollX() / ((int) (widthWithMargin * this.mPreviewRatio));
                    float pageOffset = (r19 % widthWithMargin) / widthWithMargin;
            ...
            case 2:
                ...
                if (this.mIsBeingDragged) {
                    ...
                    int width = (int) (getWidth() * this.mPreviewRatio);
    ...
    public static class LayoutParams extends ViewGroup.LayoutParams {
        ...
        public LayoutParams(Context context, AttributeSet attrs) {
            super(context, attrs);
            TypedArray a = context.obtainStyledAttributes(attrs, DraftViewPager.LAYOUT_ATTRS);
    ...
    public int getLeftOfSelectedFragment() {
        int width = getWidth();
        return ((int) (width - (width * this.mPreviewRatio))) / 2;
    }
}
